...
describe("extract-profile tests - ", () => {
  ...
  it("Email", () => {
    expect(matchEmail(makeTextItem("  hello@example.com  "))![0]).toBe(
      "hello@example.com"
    );
  });
  ...
  it("Url", () => {
    expect(matchUrl(makeTextItem("  linkedin.com/in/example  "))![0]).toBe(
      "linkedin.com/in/example"
    );
    expect(matchUrl(makeTextItem("hello@example.com"))).toBeFalsy();
  });
});
